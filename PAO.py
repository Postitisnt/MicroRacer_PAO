...
# To test the model by its own, just run this code with the final line uncommented.

###### HYPERPARAMETERS #################
"""
Please, pay attention to the fact that the number of iterations
for model training have been firstly defined to be 40000 and the first
training was done with both towers on active.
Succesfully, the iterations have been decreased and tested for the 
best performance. In this second stage, I trained the model with very low
number of iterations (min 5000) and with only one tower active at a time.
The exact alternation has not been stored, thus you can try to improve the performances
by changing the number of iterations and the activation of the towers as you like.
You can also change the batch size, over 64 are discouraged in my opinion.
"""
...
# The saved weights are the ones used in the competition, thus you can store them
# somewhere else and test your model against this one.

#Weights paths

weights_file_actor = "PAO/weights/pao_actor_model"
weights_file_critic = "PAO/weights/pao_critic_model"

# Model for easy export
class PAO:
    """
    Everything is pre-defined. To export the model
    you just need to import the class into the race.py file
    provided in the folder. Once instantiated the model, just call the 
    get_actor_model() method to get the model ready to run.
    """
    def __init__(self, model_name="PAO"):
...
def get_actor(train_acceleration=True, train_direction=True):
    ...
    out1 = layers.Dense(64, activation="swish", trainable=train_acceleration)(inputs)
    out1 = layers.Dense(32, activation="selu", trainable=train_acceleration)(out1)
    ...
    out2 = layers.Dense(64, activation="swish", trainable=train_direction)(inputs)
    out2 = layers.Dense(32, activation="selu",trainable=train_direction)(out2)
...
def get_critic():
    ...
    state_out = layers.Dense(32, activation="swish")(state_input)
    ...
    out = layers.Dense(32, activation="swish")(concat)
    out = layers.Dense(32, activation="relu")(out)
    outputs = layers.Dense(1)(out) #Outputs single value (Q)
